...
if [ "$operation" = "start" ]; then
  ...
  aws ec2 start-instances --instance-ids i-06565eba97092c4ba

  export startupStatus=$(aws ec2 describe-instance-status --instance-ids "i-06565eba97092c4ba" --output json)
  ...
  export IPAddress=$(aws ec2 describe-instances --instance-ids "i-06565eba97092c4ba" --output json | jq -r '.Reservations[].Instances[].PublicIpAddress')
#  sleep 40 | pv -t

  echo "The server has been successfully started. Starting the Minecraft server."
  aws ssm send-command --instance-ids "i-06565eba97092c4ba" --document-name "AWS-RunShellScript" --parameters '{"commands":["bash minecraft_server/scripts/start_server.sh"]}'
  ...
  aws ssm send-command --instance-ids "i-06565eba97092c4ba" --document-name "AWS-RunShellScript" --parameters '{"commands":["bash minecraft_server/scripts/kill_server.sh"]}'
  aws ec2 stop-instances --instance-ids i-06565eba97092c4ba
