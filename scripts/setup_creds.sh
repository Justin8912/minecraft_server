...
if [ "$operation" = "start" ]; then
  ...
  aws ec2 start-instances --instance-ids i-0569782c5091a09b7

  export startupStatus=$(aws ec2 describe-instance-status --instance-ids "i-0569782c5091a09b7" --output json)
  ...
  export IPAddress=$(aws ec2 describe-instances --instance-ids "i-0569782c5091a09b7" --output json | jq -r '.Reservations[].Instances[].PublicIpAddress')
  sleep 40 | pv -t
  ...
  aws ssm send-command --instance-ids "i-0569782c5091a09b7" --document-name "AWS-RunShellScript" --parameters '{"commands":["./kill_server.sh"]}'
  aws ec2 stop-instances --instance-ids i-0569782c5091a09b7
